...
let person1 = {name: "김재현", age:25, city:"seoul"};
console.log(person1);
...
console.log(keys); console.log(values1);
/* 예제 9: 객체와 조건문 활용하기
...
let html = `<table>`
// [3] data 안에 있는 요소들을 반복문으로 순회(0부터 마지막인덱스) 하기
for(let index = 0; index<= data.length-1 ; index++){ // for 반복 1번 할때마다 객체 1개 조회
  let row = data[index]; //index 번째 '동' 정보
  console.log(row.동별 , row.세대수, row["인구수(계)"], row["인구수(남)"],row["인구수(여)"]);
  //[4] 객체 정보들을 테이블로 표현하기, 표 = 배열 =<table> , 행 = 객체 = <tr> , 열 = 속성 = td
  html +=`<tr>
            <td> ${row.동별} > ${["row.세대수"]}${row["인구수(계)"]}</td> 
            <td> ${["인구수(남)"]}</td> <td> ${row["인구수(여)"]}</td> 
          </tr>`
} //for end
html += `<table>`;
document-Write(html);